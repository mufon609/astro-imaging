...
- **`--weight=noise` corpus arm** — motivated by a MEASURED 18–24% cross-night noise
  ...
  weighting is the standards-first ALTERNATIVE to exclusion and was queued BEHIND the
  ...
  closed by the owner's stop, this scalar branch stays open. **THAT GATE HAS FIRED —
  the portion rule is the chain and the frame rule a NULL — so this arm is NEXT:** one
  knob, `run_corpus_combine.sh --portion-rule --weight=noise --out=stack_<tag>.fit`
  against the canonical `_full` (same curated members, same derived reference). The
  compose's own docstring already prefers it — `run_undistort_compose.sh:105–116`:
  `nbstack` is the inverse-variance weight ONLY while every member's per-frame noise
  is the same, `noise` "yields n/s^2 in BOTH regimes … Prefer it for a MULTI-NIGHT
  compose" — so the shipped `nbstack` default contradicts the script's own note.
  PROBE FIRST, before the knob is believed to be what it claims: how Siril's per-member
  noise estimate treats a member's UNFILLED `framing=max` canvas (if coverage biases
  the weight, the knob is coverage, not noise). Read any bgnoise-based
