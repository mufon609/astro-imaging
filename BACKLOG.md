...
| `baseline_guard.py` derived summaries (corner spread, edge dipole) over Siril `stat` | a tool reports a headless PRODUCT-level regression verdict against a stored reference | **not fired** — nothing does. Every other guard checks WIRING; this is the only one that compares the product to a human-accepted prior measurement. It is a no-regression RECORD, never a quality gate — a deliberate improvement fails it and the human re-seeds with a note |
