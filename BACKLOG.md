...
| fitted lensfun entry, per lens/focal (`install_lens_model.sh`, keyed off the set record) | an upstream entry measured for THIS unit, or a chain consuming the model another way | 2026-08-05 | **not fired** — and as of 2026-08-05 the live DB is REPRODUCIBLE FROM THE REPO. It carried an arm-era incumbent (`a=0.00350093`) that existed in no tracked record, so a fresh clone could not rebuild the environment; the x86 re-fit (`july14/set-01/qa_work/lens_fit.json`) is now installed and marked, and both its inputs (`acquisition.json` + `lens_fit.json`) are tracked, so `install_lens_model.sh sessions/july14 set-01` reproduces the exact DB block. DECLARED DELTA: the two models differ by 0.468 px max / 0.344 px area-weighted RMS (0.2 px through the corner region) — below the ~3.4 px in-exposure floor and the 1.6 px paraxial band the fit exists to remove, but not zero, so a stack rebuilt now differs slightly from the ones built before this date. Re-fit per rig/lens/focal and after every `lensfun-update-data`, which wipes the patch |
...
| `baseline_guard.py` derived summaries (corner spread, edge dipole) over Siril `stat` | a tool reports a headless PRODUCT-level regression verdict against a stored reference | 2026-08-05 | **not fired** — nothing does. **But the guard is not wired: no chain, guard or web stage calls it** (only this row references it), so today it runs only by hand. It is a no-regression RECORD, never a quality gate. Blind spot to state when reading a PASS: both measures are STACK corners, which `docs/dead-ends.md` calls self-fulfilling for flat contamination — it cannot see the open `sky x V` object tilt |
