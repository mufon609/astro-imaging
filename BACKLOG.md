...
| gap | `member_separation.py` cross-match + zone medians | nothing consumes member-to-member separation (today: none — the state-change detector is unbuilt) | 2026-08-29 | **RETIRABLE** — attribution discharged (`rho_march.json`), no consumer. MEASURES only. SCOPE: zone numbers are member-frame disagreement, not delivered shape |
...
| consumer | `row_profiles.py` + `toprow_profiles.py` + `toprow_corner_coverage.py` (row-resolved member profile) | nothing cites `row_profiles.json` / `toprow_profiles.json` (today: the decision map only) | 2026-08-29 | **RETIRABLE, contested** — a DIAGNOSTIC beside its record, on no build path; all three consuming arms are closed. Keep only as the measurements' home |
