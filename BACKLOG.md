...
| `scripts/calibrate/solve_field.py` coverage rescue rung — re-solves on the largest centred box inside Siril's measured covered rectangle when a blind solve starves | the astrometry.net engine accepts a detection-region/subarea constraint of its own | 2026-08-19 | **ROW + CONDITION NEWLY AUTHORED — the rung shipped (fa8db6f) with a LIMITS block and no retirement trigger; RATIFIED (owner 2026-08-19).** Not fired. Standing: a GENERAL SAFETY NET, not the fix for the corpus starvation (the reference derivation was — f7512f1); fires only on NO SOLUTION or floor-class, keeps the strictly better result, soft-by-contract |
...
- **The per-member trim — RULED, RAN, REFUTED; no trim ships.** Owner 2026-08-22, verbatim: trim
  *"each side by about 5% ... so the worse part of each image never makes it into the stack"* — a
  ...
  (`datasets/aug09/smear_work/rho_march.json`); executed 690746c, 2654d31, 9918f01. The owner's
  ...
  *"the stars being stacked are the worse images possible"* (measured on that axis: member-own field
  ...
  9918f01; cross-chain bow 31.5 → 1.4 px) and `-output_norm`'s single-pixel zero point (the closed
  ...
  `datasets/corpus/crop_work/`; the arms were disposed in the rig cleanup (4faf181), the records
...
4. **Compose-input edge shrink / min framing** — ships less sky rather than fixing the
   ...
   (per-member edge trim) was an owner-DIRECTED TEST (2026-08-22) that RAN (690746c)
...
- **L1 background level — the FOCUS item (user-ratified), and no longer a
  ...
  **THE COLLISION IS SETTLED BY MEASUREMENT: the per-member trim was an
  owner-DIRECTED TEST (2026-08-22) that RAN (690746c) and is REFUTED — no trim
  ...
  mild ~5%/side bracket was the DIRECTED TEST that RAN (690746c) and failed — it
...
2. **A few-degree axis offset between the CFA and debayered grids**, pre-registered
   ...
   **THIS IS AN ABANDONMENT THAT ITS OWN DESIGN PRE-AUTHORISED, AND THE DISTINCTION
   IS WORTH MORE THAN THE ITEM.** `08a6840` declared, BEFORE the run and before
...
- **BANDAID / accepted failure mode, NOT candidates:** PSF homogenisation, zone
  ...
  (owner 2026-08-22) as a DIRECTED TEST; the test RAN (690746c) and REFUTED it** —
...
**SEPARATE from the record-vs-reality defect** (fixed at `da3296c`; the mechanism
...
defined anywhere — the sentence entered at `da3296c` already dangling; one
...
wart: the promote of the member-selection canonical (d661715) wrote
...
`git show 7079c63:scripts/setup/site.json`), plus the
...
**THE HISTORY — RULED AND EXECUTED (owner 2026-08-30, verbatim: "let's try and right
this wrong").** The literal had entered at `85f5030` and `86ec80d`, after the last push
`048e69d`, and sat in the trees of 414 unpushed commits and in 6 commit messages; no
commit at or before the push carried any form. `git filter-repo` 2.47.0
(`--replace-text` + `--replace-message`, 12 EXACT forms — a near-literal prefix would
have hit three tracked Siril star lists a digit away from the site, measured by the
census over every commit) rewrote the history: 1142 commits → 1142, 421 rewritten and
721 unchanged, every form → a `REDACTED_*` token; afterwards **0 hits for every form
over every ref's blobs and every message**, `git fsck` clean, and HEAD's tree
byte-identical to the pre-rewrite tree (`ba055dc4…` — the rewrite changed no current
file). Every hash after `048e69d` changed: prose citations in the tracked `*.md` were
remapped (58 over 9 files); records, ledgers and `PIPEREV` header values keep the old
hashes — the products' FITS headers carry them forever — and
`datasets/corpus/history_rewrite/commit_map.tsv` (1142 rows) resolves any old hash.
The pre-rewrite objects survive only in two local backups that carry the site (a
bundle and a mirror clone, both outside the repo). Record:
`datasets/corpus/history_rewrite/site_privacy_history_rewrite.json`; ledger
`site_privacy_history_rewrite`. The push is a fast-forward from `048e69d`, made after
the worker's independent verification of the rewrite.

**Closes when** the push has landed with the guard green — the process (the config,
the resolver, the guard, the setup step) and the rewrite are done; what remains open
is the from-frames DERIVATION as the transcription's verify step (degree-level, needs
per-frame solves; the register row for `check_site_privacy.py` names it as the
retirement path).
...
candidate at e5c1cd4, the promote at d661715) and the read-back is DONE:
...
a `docs/dead-ends/siril-behaviors.md` entry. Stages 0–2 SHIPPED: runner d795ec9,
curves ffa2203, recipes 7bdd51d, the 22 re-calibrations 6564f24.
...
**Delta declared (6564f24; `datasets/corpus/spcc_pin_zf/pin_record.json`).** All
