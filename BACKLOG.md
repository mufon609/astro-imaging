...
## `site-privacy-vs-public-repo` — CLOSED 2026-08-30: the site is a gitignored local config, the guard derives every form, the history was rewritten in two passes, and the repo is pushed
...
`git show 60de095:scripts/setup/site.json`), plus the
...
this wrong").** The literal had entered at `3085b79` and `86351d9`, after the last push
...
file). **PASS 2, forced by the second reader:** the worker's independent verification
(its own scanner over all 6885 objects and every message, with the guard's derived
forms) found a DERIVED class the exact forms missed — the perturbed values
`verify_site.py`'s falsification test carries (the real pair + 0.63° / + 0.09° / ÷ 10,
one documented arithmetic step from the site) in 8 historical blobs over 3 paths and 1
message, 416 post-push commits, HEAD clean. A second `filter-repo` pass with 4 exact
perturbed forms (both transcriptions each): 1143 → 1143 commits, 422 rewritten, HEAD's
tree (`1711aeac…`) unchanged by the pass; afterwards **0 hits for all 16 patterns of
both passes over every ref and every message**. The guard now derives the perturbed
forms too (a value one documented step from the site is the site). Every hash after
`048e69d` changed: prose citations in the tracked `*.md` were remapped after each pass
(58, then 57, over 9 files); records, ledgers and `PIPEREV` header values keep the old
hashes — the products' FITS headers carry them forever — and
`datasets/corpus/history_rewrite/commit_map_pass2.tsv` (filter-repo's CUMULATIVE map:
ORIGINAL → FINAL, 1143 rows; `commit_map.tsv` is the pass-1 original → intermediate
snapshot) resolves any old hash. The pre-rewrite objects survive only in two local
backups that carry the site (a bundle and a mirror clone, both outside the repo).
Record: `datasets/corpus/history_rewrite/site_privacy_history_rewrite.json`; ledger
`site_privacy_history_rewrite` (two lines). The push is a fast-forward from `048e69d`,
made after the worker's verification of the FINAL history.

**CLOSED — the push landed 2026-08-30:** `048e69d..f4907e3 main → main`, 433 commits,
a fast-forward, the pre-push hook's 33 guards green (`check_site_privacy` 0/0 on the
tree), the remote head equal to the local HEAD. What remains is not this item's: the
from-frames DERIVATION as the transcription's verify step (degree-level, needs
per-frame solves; the register row for `check_site_privacy.py` names it as the
retirement path), and the guard's stated limits (forms it does not derive: an
undocumented offset, UTM/MGRS/what3words).
