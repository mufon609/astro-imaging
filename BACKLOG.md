...
| gap | `member_separation.py` cross-match + zone medians | nothing consumes member-to-member separation (today: none — the state-change detector is unbuilt) | 2026-08-29 | **RETIRABLE ON THE GATE** — attribution discharged (`rho_march.json`), no consumer. MEASURES only. SCOPE: zone numbers are member-frame disagreement, not delivered shape |
...
| data | `zero_point.py` ZP arithmetic (`MAG_VT − m_inst` + median/flatness fit) | an absolute throughput calibration for this camera+lens exists on the rig, or the corpus gains two nominal exposures on one night | 2026-08-13 | **RETIRABLE ON THE GATE** — question CLOSED as underpowered, ZP homed in `TOOLS.md`, no citer. astrometry.net supplies the match and `MAG_VT`, so no in-house cross-matcher |
...
| consumer | `row_profiles.py` + `toprow_profiles.py` + `toprow_corner_coverage.py` (row-resolved member profile) | nothing cites `row_profiles.json` / `toprow_profiles.json` (today: the decision map only) | 2026-08-29 | **RETIRABLE ON THE GATE, contested** — a DIAGNOSTIC beside its record, on no build path; all three consuming arms are closed. Keep only as the measurements' home |
