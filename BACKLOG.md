...
| `scripts/lib/astrometrics.py` hand-rolled FITS parse — the 2880-block header walk + `FOCALLEN`/`XPIXSZ` regex behind `read_fits` and `fits_pixel_scale`, which feed the plate-solve extraction | astropy is available on the target rig and the parse moves to it | 2026-08-16 | **FIRED at `db3733b`, and this is the condition's FIRST row — it had none.** Evidence at `bcfe859`: `grep -n 'from astropy.io import fits' scripts/lib/astrometrics.py` → **lines 23 and 48** (`read_fits` → `fits.open`, `fits_pixel_scale` → `fits.getheader`), and `git grep -n '2880' -- '*.py' '*.sh'` → **rc=1, no match**, so no hand-rolled FITS block parser survives anywhere in tracked code and `db3733b`'s own *"2 of 5 parsers done"* is complete for this class. **THE DECLARATION WAS DELETED BY THE COMMIT THAT FIRED IT** — `db3733b` removed the docstring clause *"the hand-rolled FITS parse itself retires to astropy (installed on this rig — an arm-doable retirement, BACKLOG item 6)"*, whose pointer was already dead (numbered items are the retired scheme). So the file has never held the literal (`git log -S` for it over that path returns empty) and matches `check_removal_conditions.sh`'s `DECL_PAT` **0** times: this is the register's own hole **(b) NO-CONDITION-ANYWHERE**, live in the tree, and the second detector is UNBUILT. **This row does NOT make the guard able to see it** — `NDECL` reads **30** and exit **0** both with and without the row, and the file is in neither run's declaring set. What survived the deletion was a POINTER: `README.md`'s repo-map row read *"the FITS parse itself moves to astropy on the target rig (removal register)"* — true when written, false from `db3733b`, citing a register row that never existed, and the only site tree-wide (`git grep -ci 'moves to astropy'` → 1 file); corrected in the same commit as this row. **DISPOSITION NOT SETTLED HERE:** whether a discharged row is removed or retained is the owner-held schema question (`pending-owner` 1, *"a destination for code that outlives its divergence"*); `243b0a6` is the precedent for removal |
| `scripts/qa/fit_ptlens_joint.py` joint ptlens(a,b,c) + distortion-centre least squares with a projective nuisance | hugin/lensfun fit ptlens + distortion centre jointly against an absolute (catalogue) reference, or no OPEN item in `one-sided-band` / `corner-fix-landscape` still consumes a fitted distortion-centre quantity | 2026-08-19 | **ROW + CONDITION NEWLY AUTHORED — this divergence shipped with NONE** (no `REMOVAL CONDITION` literal anywhere, no row: the register's hole (b) NO-CONDITION-ANYWHERE, live in the tree until now; found by audit, the third instance beside `psf_calib.py`'s precedent). **PENDING OWNER RATIFICATION.** Not fired: hugin's own d,e stage diverges (d = 6.3e6, the file's docstring) and both consuming items are OPEN. Not invoked by any chain; its model reaches production only through the explicit user-judged promote path (`fit_lens_model.sh`) |
| `scripts/darktable/cp_coverage.py` control-point radial-coverage analysis (rho percentiles + the pre-registered corner-true criterion) | hugin/lensfun report per-radius control-point support against the model's own normalisation, or the fitting route pins control points to a corner-inclusive station grid by construction | 2026-08-19 | **ROW + CONDITION NEWLY AUTHORED — shipped with none; hole (b); PENDING OWNER RATIFICATION.** Not fired. Imported by `fit_lens_model.sh` only; the CLI's corner-support gate-exit has NO caller (README: imported, not invoked), so the exit-1 path is dead in practice and the analysis is promote-path evidence |
| `scripts/calibrate/spcc_cone.py` hand-rolled nside=2 nested ang2pix cover + `_tan_pix2sky` gnomonic step | (a) cover: siril 1.5 `healpix` adopted AND its pixel list verified to map to the zenodo chunk names (`siril-1.5`), or `astropy_healpix` adopted into this script's interpreter (installed in `/opt/astro-venv`, ABSENT from host python3 — `TOOLS.md`); (b) projection: the step moves to astropy WCS (already imported in this file; used for exactly this in `derive_compose_ref.py`) | 2026-08-19 | **ROW + CONDITIONS NEWLY AUTHORED — shipped with none; hole (b); PENDING OWNER RATIFICATION. Two clauses, evaluated separately (rule 6).** Clause (b) is satisfiable NOW and is a DECISION TO WORK, not an event — SELF-GATED per the register's own taxonomy. Consequence bound: chunk SELECTION only, sub-arcmin error against ~29 deg pixels, and siril names any missing chunk loudly, so a wrong cover cannot silently corrupt anything |
| `scripts/stack/lens_preflight.py` pinned-model XML scan — reads the lensfun user-DB XML as TEXT and compares literal a/b/c for the exact lens@focal (deliberately not the fuzzy matcher, which stays the tool's) | lensfun/darktable expose a headless query of the INSTALLED model's coefficients for a given lens@focal, or the chain consumes the model other than through the lensfun user DB | 2026-08-19 | **ROW + CONDITION NEWLY AUTHORED — the leg declared none; PENDING OWNER RATIFICATION.** Not fired: Debian ships no lensfun query CLI (`lenstool` unpackaged, `python3-lensfun` exposes DB-path helpers only, `liblensfun-bin` update/adapter utilities only — the file's own probe list) |
| `scripts/calibrate/solve_field.py` coverage rescue rung — re-solves on the largest centred box inside Siril's measured covered rectangle when a blind solve starves | the astrometry.net engine accepts a detection-region/subarea constraint of its own | 2026-08-19 | **ROW + CONDITION NEWLY AUTHORED — the rung shipped (a515053) with a LIMITS block and no retirement trigger; PENDING OWNER RATIFICATION.** Not fired. Standing: a GENERAL SAFETY NET, not the fix for the corpus starvation (the reference derivation was — bf3491e); fires only on NO SOLUTION or floor-class, keeps the strictly better result, soft-by-contract |
