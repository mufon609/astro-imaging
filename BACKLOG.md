...
| fitted lensfun entry, per lens/focal (`install_lens_model.sh`, keyed off the set record; the corpus currently shoots one lens) | an upstream entry measured for THIS unit, or a chain consuming the model another way | 2026-08-05 | **not fired** — live DB carries the incumbent `a=0.00350093`; the x86 re-fit (`a=0.0033627`, `qa_work/lens_fit.json`) CONFIRMED it within 0.468 px and was deliberately not installed. Re-fit per rig/lens/focal and after every `lensfun-update-data` |
