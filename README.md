...
| `install_lens_model.sh` | installs the PINNED distortion model (`scripts/darktable/lens_models.json` — the authority) into the live lensfun user DB AND strips that lens's `<vignetting>`/`<tca>`, the distortion-only enforcement point. Lens + focal come from the set's `acquisition.json`; the DB FILE is found by searching for the lens, since vendor file names are lensfun's business. `--from-fit` installs a set's fresh `lens_fit.json` instead, for an A/B — a fresh fit is a CANDIDATE until it is pinned. Records what it replaced in an XML marker, and a different fitted entry STOPS rather than being silently overwritten. Machine-local like `lensfun-update-data`, which WIPES the patch: re-run after every DB update |
