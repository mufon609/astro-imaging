...
| `install_lens_model.sh` | installs a SET'S OWN optical-state model (`qa_work/lens_fit.json` — THE authority: fitted from the set's frames, or carrying explicit `inherited_from` provenance) into the live lensfun user DB AND strips that lens's `<vignetting>`/`<tca>`, the distortion-only enforcement point. The model keys on the OPTICAL STATE, per set — focus recalibrates every session, and five fitted states measured pairwise all exceed the 0.47 px equivalence bound; the repo-global pinned model (`lens_models.json`) is REMOVED (it was right for at most one state per night; git history carries its provenance). Lens + focal come from the set's `acquisition.json`; the DB FILE is found by searching for the lens. Records what it replaced in an XML marker; a different fitted entry needs `--replace` (the chain passes it — per-set swapping is the routine). Machine-local like `lensfun-update-data`, which WIPES the patch: the chain re-installs per run |
