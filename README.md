...
| `install_lens_model.sh` | installs the model FITTED FROM A SET'S OWN FRAMES into the live lensfun user DB AND strips that lens's `<vignetting>`/`<tca>` — the distortion-only enforcement point, since darktable ignores a style's lens op_params. Lens, focal and coefficients come from the set's records (`acquisition.json` + `qa_work/lens_fit.json`); the DB FILE is found by searching for the lens, since vendor file names are lensfun's business. Records what it replaced in an XML marker, so the patch is self-describing and a different fitted entry STOPS instead of being silently overwritten. Machine-local like `lensfun-update-data` — re-run per rig, per lens/focal, and after every DB update; verify with `verify_lens_card.py` |
