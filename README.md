...
| `scripts/setup/site.example.json` → `site.local.json` | the OBSERVING SITE — latitude, longitude, optional elevation; the geocentric triple derives at runtime. A home address in a tree meant for publication, so the numbers live ONLY in the gitignored `site.local.json` (copy the template; positive-EAST longitude; the map-URL trap is in the template; a session shot elsewhere gets `sessions/<session>/site.local.json`). Load-bearing science, not metadata: `scripts/lib/acquisition.py` is the one loader, and hour angle, altitude, azimuth and parallactic angle derive from it. No tracked file carries a coordinate in any form — a record's `site` block holds the config's sha256 + provenance, horizon-frame quantities are written at whole degrees, and `scripts/qa/check_site_privacy.py` (`run_guards` + the pre-push hook) goes RED on any form in the tree or the index; a missing config is a null block that says so. The value is TRANSCRIBED and UNVERIFIED; `verify_site.py` bounds it at the degree level only. Mechanism + rulings: `docs/dead-ends/evidence-provenance.md`, "THE OBSERVING SITE IS A HOME ADDRESS" |
