...
| file | role | status |
|---|---|---|
| `acquisition.json` | how the set was SHOT: `exif` (camera/lens/focal/exposure/ISO/FOV+pixel-scale/cadence — auto-derived by `scripts/lib/acquisition.py`, do not hand-edit) + `mount` (`fixed` \| `tracked` — the one fact EXIF can't record, human-declared). A consumer needing the camera model (e.g. the anomaly audit's cross-frame linking) resolves this FIRST and STOPS if `mount` is undeclared — no silent default. `site` is PROVENANCE ONLY: which gitignored site config resolved (`resolved_from`), its sha256, status / verified, whether an elevation was recorded — NO coordinate in any form (the site is a home address; the numbers live in `site.local.json`, read at runtime through `scripts/lib/acquisition.py`; `scripts/qa/check_site_privacy.py` guards the tree; a missing config = a null block that says so) | DURABLE — `acquisition.resolve()` seeds it + gates on it |
...
| `qa_work/star_shape_*.json` | Siril `seqtilt`'s spatial star-shape report for a stack — off-axis aberration (centre vs corners = the RADIAL term) + sensor tilt (best vs worst corner = the ASYMMETRIC term), written by `git show d374450:scripts/qa/star_shape.py`. The tool measures; this only records | DURABLE — the versioned result; regenerated by a re-run |
| `geometry.json` | per-set composition facts: terrestrial `foreground` (rect \| npz `mask` path, session-relative), optional `starsep` overrides. No file = foreground **none** (the whole frame is eligible sky); a new set NEVER inherits another set's foreground silently. A configured foreground must TOUCH A FRAME BORDER — terrestrial obstructions are border-anchored by construction, and the foreground is excluded from the measured sky scope, so a floating interior one would carve graded sky out of that scope — refused loudly at configure time. The background is NOT a composition fact: sky scope is selected STATISTICALLY (dark blocks, foreground excluded), so no galactic band or object region is ever configured per set — a bright object has no fixed geometry a mask could scope (`docs/dead-ends/measurement-discipline.md`, the composition-agnostic sky-scope entry) | DURABLE — resolved by `astrometrics.configure()` in every product entry point |
...
| `recipe.json` | the per-dataset processing knobs + optional `spcc` spec (broadband sensor/filter names, or narrowband `rwl`/`gwl`/`bwl` wavelengths; SPCC `-atmos` stays OFF unless site elevation/pressure are declared — they are not derivable) + optional `frame_qa` / `stack` policy blocks. Every block is present only when the set needs it (every canonical set carries a `spcc` block pinning the Nikon Z f proxy — a named sensor is REQUIRED, `spcc_run.py` refuses a spec-less run: no generic default exists, `docs/spcc-sensor-curve-z6iii.md` §1.2) | MIXED — the `spcc`/`frame_qa`/`stack` blocks are durable; the **`render` knob block is PENDING the render-tier build** (the old schema died with the wiped chain; the rebuild re-seeds it — user-gated, starts on this rig: BACKLOG) |
...
| `baseline.json` | the no-regression record: the TOOL-reported measures of the product a human accepted (Siril `stat` corner spread, edge dipole, per-channel centre level — the level rows are ADVISORY, shown not counted, while the product's `STACKNRM` differs from the baseline's: the anchor changed by design, and re-seeding on acceptance re-arms them; the absolute corner-spread ceiling WARNS — `CEILING … EXAMINE THE IMAGE MANUALLY` — and never stops, owner-directed after it misfired on aug14/set-05's Milky Way gradient, while the over-baseline, dipole and level rules stay hard), its tolerances, and a head+tail+size hash of the stack they were taken from | DURABLE — written ONLY by `scripts/qa/baseline_guard.py --seed` (never by hand); `run_set_chain.sh` compares every finished product against it and exits 8 on a regression. Seeded for july31 set-01..04 |
| `../GENERIC.json` | the repo-wide base layer of generic render knobs | STUB — re-seeded by the render-tier build's laddered knobs |
| `datasets/corpus/` (not per set) | the CROSS-NIGHT COMBINE's records — `recipe.json` (the member-selection constants, `member_selection.portion_rule`), `baseline.json` (the corpus product's OWN no-regression slot, `baseline_guard.py --baseline=datasets/corpus/baseline.json`, regions placed inside the coverage rectangle because a union's corners are empty), `member_selection/` (the stage records `<tag>_portion.json`, the profile cache, the acceptance / candidate / promote records), `qa_work/` (the guard's Siril scratch records), plus the cross-session audits indexed in `datasets/corpus/README.md` | DURABLE — written by `run_corpus_combine.sh --portion-rule`, `run_member_crop.sh`, `baseline_guard.py --seed` |
