...
| `qa_work/registration_qa.json` | the registration route's record for the set: the field solve, the mount verification, the in-exposure floor, the route experiments + their verdicts, and the shipped render | **NOT DURABLE AND NOT PRODUCED — HISTORICAL.** No instance exists anywhere in the tree and no tracked script writes one: `star_shape.py` takes a caller-supplied `--json=<out.json>` and no script invokes it at all, while `fingerprint.py`'s two mentions are docstring citations of july14 numbers, not writes. The one instance this name ever had was july14/set-01, whose session is archived — which is why `TOOLS.md` and `docs/wide-field-untracked-registration.md` still cite it. Kept as a row so those citations resolve; **a new dataset does NOT get this file** |
