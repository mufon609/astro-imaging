...
| `composition.json` | how a multi-line/multi-filter target's composed linear stack is BUILT: `kind` (`dualband-osc`; mono-filters when that lands), extraction params, `lines`, and the `channels` palette mapping. Build-side facts ONLY — calibration/render knobs stay in `recipe.json`. Absent file = the set processes as an ordinary single stack (degrade-loudly rule) | `run_pipeline.sh` (ingest fork) + `scripts/stack/compose.py` |
