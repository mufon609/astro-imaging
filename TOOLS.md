...
| **GraXpert classical interpolators** (RBF / Splines / Kriging via `-preferences_file`, no AI model) | **INSTALLED and REACHABLE HEADLESS — MEASURED from source at tag 3.0.2, matching the installed build.** `cmdline_tools.py` reads `interpol_type_option` and `background_points` from the `-preferences_file` JSON and passes both to `extract_background`; our own `~/.config/GraXpert/preferences.json` already carries the schema (`interpol_type_option`, `RBF_kernel`, `spline_order`, `bg_pts_option`, `bg_tol_option`, `smoothing_option`). **The CLI default is AI**, so the measured ~2/3 absorption was the default path. **The CLI does NOT generate sample points** — no `background_grid_selection` call in it — and the "not enough points" guards are GUI-only (`app.py` `messagebox.showerror`), so **a headless classical run with an empty `background_points` has no guard at all**. Points can be produced by GraXpert's own `background_grid_selection(data, num_pts_per_row, tol, sample_size)`, so the route needs no in-house analysis. | **OPEN — and the question moved from "is it usable?" to "where do the points go?"** MECHANISM, untested: the AI path infers a background CLASS and absorbs frame-filling structure with no knob to refuse it; RBF/Splines/Kriging have no notion of background and fit a surface through the points supplied, so absorption depends on placement and stiffness — a failure that is inspectable and controllable where the AI one is not. **LIMIT: auto-placement does NOT escape it** — `background_grid_selection` imports `find_darkest_quadrant`, so on a frame-filling starlight field the darkest cells are still starlight and auto-placed points land on the structure. **The route is open on DELIBERATE placement, not automatic.** PROBE (specified, unrun): one knob on the same stack — default AI vs `-preferences_file` RBF with stated density/tolerance — judged on NAN-region contrast %R/G/B, comparable to the recorded 8.5/2.9/5.6 (own sky flat) and 2.4/0.7/2.1 (AI Division); a third arm with hand-placed points outside the nebulosity separates placement from method. NOT CHECKED: that `background_grid_selection` imports from a pip install rather than only the bundled app. |
