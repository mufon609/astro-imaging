...
|---|---|---|---|---|
| **Siril** (calibrate/register/stack, `seqextract_HaOIII`, drizzle) | FREE | siril-native | ✅ / ✅ / ✅ | **DUAL-BAND SIZING, probed on 1.4.4 — full-size OIII is the DEFAULT, not a gap to engineer around:** `extract_HaOIII [-resample={ha|oiii}]`, and *"if this argument is not provided, no resampling will be carried out and the OIII image will have twice the height and width of the Ha image"*. Downsampling OIII happens only on `-resample=oiii`; `-resample=ha` interpolates Ha up instead. It consumes the CFA image BEFORE debayer, so it never meets the undistort route's debayer requirement. **Default.** One integrated FOSS workflow, scriptable headless, 32-bit, drizzle + Bayer-drizzle, dual-band line extraction. What our `run_pipeline.sh` orchestrates. |
...
| **darktable + lensfun** (`darktable-cli --style <s> --style-overwrite`) | FREE | CLI | ✅ / ✅ / ✅ | **THE ADOPTED FIX for this class — measured WIN at mid/edge, in production, shipped — with one measured CLASS LIMIT: the community profile's paraxial error writes an along-drift CENTRE BAND into a far-drifting set** (shipped render centre station 5.30 px / roundness 0.480 vs 3.60 / 0.706 perpendicular; the no-model control's centre is its BEST region — `docs/dead-ends.md` paraxial-band entry). **Fixed on this rig by a FITTED entry, PINNED per lens@focal** (`scripts/darktable/lens_models.json` — the authority; a per-set fit is a CANDIDATE promoted by an explicit act). The per-set variant was tried and REFUTED at its root (`docs/dead-ends.md`): its founding number was a compose artifact, the 0.47 px equivalence bound is exceeded 7-23x by REFITS OF ONE SET (0.36-6.30 px), and per-set models broke the combine — 2.99 px corner disagreement within a night, 5.34 px across nights, star doubling failed by eye, against 0.14-0.93 px under one model. The chain installs the pinned entry per run; the preflight and `lensfun-update-data` are the per-rig setup steps. An OFFICIAL *measured* lens profile, immune to the index-sparsity that kills a per-frame SIP fit. darktable must be built against Lensfun (Debian's is). **THE PARENTHETICAL THAT USED TO SIT HERE — *"Debian's RawTherapee is NOT — it doesn't link lensfun, so its auto-match is unavailable"* — LOOKS FALSE, AND IT WAS CLOSING A ROUTE.** MEASURED: `apt-cache show rawtherapee` → **5.12-2+b1**, `Depends: … liblensfun1 (>= 1:0.3.4) …`. **A Debian package does not carry a VERSIONED hard dependency on a shared library it does not link** — `dpkg-shlibdeps` generates that line by scanning the built binary's ELF `NEEDED` entries, and the versioned form is that mechanism's signature. **INSTRUMENT LIMIT, stated because this is the exact shape that produced the SCAMP and PSFEx errors in the other direction: this is PACKAGE METADATA, not a running `ldd`** — RawTherapee is not installed here. The completing check is `ldd $(which rawtherapee) | grep lensfun` after an install, or a contents lookup. **Rated high-confidence-FALSE rather than settled.** Whether RawTherapee is *worth* anything on this route is a separate question nobody has asked; what is recorded is that the stated ground for excluding it does not appear to hold. On july14, Siril `seqtilt` control → corrected → community 168-fr control: **off-axis aberration 0.57→0.31→0.25 px**, stars 5,095→10,707→11,805, 54/54 register; the SHIPPED fitted-model render measures **3.06 px truncated-mean / 12,976 stars / sensor tilt 0.31 (10%)** (the band leaving the statistic, not the floor moving). Sharpness vs the community model is NULL (truncated mean FWHM 3.20→3.28) and a radial model cannot fix the one-sided term (0.50→0.42→0.51 across the community arms; the fitted model's cut to 0.31 was paraxial model error, not tilt) — claim carefully. **The style is pinned in-repo** (`scripts/darktable/*.dtstyle` + `install_styles.sh`, verified to reproduce the warp to 0.000 px) — no GUI step. **`--style-overwrite` is REQUIRED**, else the style is silently ignored. **Correction set: a style's lens op_params are IGNORED — `modify_flags` included** (measured: flags 0–7, method/inverse flips, a blanked lens string → byte-identical output; `docs/dead-ends.md`) — and darktable's per-image default set includes **vignetting**, which FIGHTS a master/sky flat (the measured double-correction bowl, `git show aac7c18:datasets/july14/set-01/qa_work/gradient_qa.json` — the retired july14 exemplar, tag `july14-all5-cov25frame-approved`); **distortion-only is enforced in the lensfun user DB** (`install_lens_model.sh` strips this lens's vignetting/tca; verify with a uniform-card warp — corner medians == centre). **ICC is per-LEG, do not cross them: the 32-bit float production leg ships the TIFF untagged and exports `--icc-type LIN_REC709` (measured identity, ratio 1.0000 at every level and channel); `--icc-type SRGB` belongs ONLY on the 8/16-bit probe legs where it matches Siril's own tag** (`docs/dead-ends.md` ICC entry; CLAUDE.md environment). **The style carries ONLY the enabled bit; `focal`, `scale`, `camera` and `lens` are all baked but IGNORED — darktable re-detects them from EXIF and recomputes the autoscale (MEASURED: focal 70 vs 24 give opposite-sign warps; scale 1.046 vs 0 vs 1.5 are identical to 0.000 px; a swapped lens string gets that lens's own profile). So ONE style is camera-, lens- and focal-general.** **The same mechanism is a trap: darktable NEVER FAILS** — an unmatched lens gets NO correction, silently (0.000 px over 413 stars, exit 0, nothing in the log), and a wrong-but-present lens gets a wrong model just as quietly. It cannot be relied on to degrade loudly: the CHAIN must assert EXIF camera+lens+focal against the DB per set and STOP on a miss ("did the warp happen" is not enough — it passes the wrong-lens case). Debian's lensfun 0.3.4 lacks the Z6III → **`lensfun-update-data`**, which ships in **`liblensfun-bin`** (NOT `python3-lensfun` — that package exposes only DB-path helpers and no matcher); it writes the upstream DB to `~/.local/share/lensfun/updates/version_1`, a **machine-local, untracked** path the route depends on and which does not migrate with the repo — re-run it per rig. There is **no lensfun query CLI** in Debian (`lenstool` is unpackaged), which is why `scripts/stack/lens_preflight.py` proves the correction by asking darktable rather than by querying lensfun. Deterministic in pixels; between two measured runs its TIFF differed by one metadata byte — never gate this route on a file hash. Ordering is load-bearing: calibrate in SENSOR space → debayer → warp → register. **DECENTRING — what this stack can and cannot express, all verified against the installed 0.3.4:** the radial models (`poly3`/`poly5`/`ptlens`) are functions of radius alone and cannot represent a left-right asymmetry by construction; `acm` (Adobe Camera Model) is the only lensfun model carrying Brown's tangential pair (k4,k5) and **does not exist in 0.3.4** (zero occurrences of `ACM` in the v0.3.2 and v0.3.4 sources, none in the installed `liblensfun.so.0.3.4`; nine in v0.3.95, and there only in the correcting direction). The `<center>` element IS present and applied in 0.3.4, and darktable honours it — but the measured lens decentring is ~180–210 px in x and ~210–240 px in y, and installing that centre on coefficients fitted for centre=0 is a LOSS at every sign (2.589 → 4.235–7.610 px residual). Both routes are registered in `docs/dead-ends.md`; the only one that can work is a JOINT refit of a,b,c about a free centre. |
| **Montage** (`mProjectPP`/`mProject`/`mAdd`, IPAC/Caltech) — **NOT INSTALLED, and it is one `apt install` away** | FREE | CLI | ✅ / ✅ / ✅ (packaged; **UNPROBED here**) | **The gap SWarp's row leaves, at the same tier. NEVER EVALUATED rather than rejected — provenance in the note under this table.** CLAIM (the ORACLE's, **DOCTRINE — source read, PINNED, NOT probed on this rig**): Montage READS AND APPLIES per-image SIP, where SWarp accepts a TAN-SIP header and silently drops the distortion (the SWarp row above, measured). **PINNED at `Caltech-IPAC/Montage`** — the whole SIP chain (`distortinit` `wcsinit.c:583`, `setdistcode` `wcs.c:592`, `pix2foc`/`foc2pix` `wcs.c:2123`/`:2302`, polynomial body `distort.c:289`) sits inside **`82a5e1162c6c` (2016-12-22)**, and the per-image-reprojection claim in `montageProject.c` **`6a34acf914b3`** / `montageProjectPP.c` **`5d2f1a1583ad`** (2023). **THOSE COORDINATES DESCRIBE THE UPSTREAM BUNDLED TREE, WHICH DEBIAN STRIPS — label which build any Montage SIP citation refers to.** The `+dfsg` repack removes all three bundled WCS trees and the binary depends on `libwcstools1t64` from `wcstools 3.9.7-2`; the ORACLE reports that package carrying `distortinit`/`pix2foc`/`foc2pix`, the `DISTORT_SIRTF` key and `A_ORDER`/`AP_ORDER` reads, so **the capability survives the repack** and the governing artifact for the apt route is `wcstools 3.9.7-2` `libwcs/{distort,wcs,wcsinit}.c` rather than anything under `82a5e1162c6c`. **FOUR BOUNDS TRAVEL WITH THAT AND NONE IS DISCHARGED:** the `wcs.c` read was ~80% PARTIAL, so every finding drawn from it is a POSITIVE that survives truncation and **every NEGATIVE from that file is withheld**; it read `montageProject.c` and NOT `montageProjectPP.c`, so the answer covers `mProject` only and the plane-to-plane fast path is unexamined; **nothing was EXECUTED** — whether the shipped `libwcstools1t64` exports these symbols is one `nm -D` away and needs an install, which is the fork that was not crossed; and the two seats that produced it did not check each other before reporting. **A TRAP WORTH MORE THAN THE FINDING: `montageProject.c` names NO distortion function at all** — the chain runs `wcsinit`→`distortinit` and `pix2wcs`→`pix2foc` inside Mink's library — so **grepping Montage's own source for `pix2foc` returns ZERO while the capability is in use**, a confident false negative available to anyone who checks the obvious way on a route-bearing question. Coordinates are quoted here because a SHA makes them stable — the opposite of this repo's own never-cite-line-numbers rule, which is about a tree that moves. **The read itself was taken on `master`**, against the Oracle's own pinned-SHA rule; the files not having moved since 2016 is what rescues it, and that is stated rather than smoothed over. **NAMING TRAP THAT MAKES A GREP LIE: the key it looks for is `DISTORT_SIRTF`, so searching its source for "SIP" returns a FALSE NEGATIVE.** Same shape as `SWarp` vs `swarp` one row down and as `DISTORT_` vs `undistort_` in this repo's own sweeps. **THE BOUND THAT STOOD HERE IS DELETED — IT WAS FALSE ON BOTH CLAUSES AND IT WAS GUARDING ITS OWN CLOSURE.** It read that which of three bundled WCS trees links is *"a `config.make` read on the BUILT source, so it is unanswerable from upstream history and unanswered here"*. **`config.make` IS tracked upstream and its entire content is `montage_prefix=/usr/local`** — an install prefix naming no WCS tree, corroborated free by the blob being exactly 25 bytes for a 25-character line. **And it IS answerable from upstream history:** `lib/src/Makefile` at blob `c67b08e8cab764be2d729e30bac525c01b624469` (31 lines) has two `wcs` matches, both `wcssubs3.9.0_montage`, with `wcstools-3.8.1` and `-3.8.7` appearing ZERO times; line 11 is the linkage rather than mere descent, building that tree and copying `libwcs.a` and its headers into `lib/`. Cite the BLOB, not the line numbers — coordinates decay, which this row has already demonstrated once. **BUT THE ANSWER DOES NOT TRANSFER TO THE ONLY PROCUREMENT ROUTE ON THE TABLE, and that is the sharper half:** `apt-cache showsrc montage` gives `Build-Depends: … libwcstools-dev …`, so **Debian's build links the SYSTEM wcstools and NONE of the three bundled trees** — and this is now confirmed on BOTH SIDES OF THIS RIG'S SUITE SPLIT, which matters because the local `showsrc` reads bookworm: `libwcstools-dev` is in `Build-Depends` at **6.0+dfsg-7** (bookworm source, read here) AND at **6.1+dfsg-2** (the parent of the kali-rolling binary candidate, read upstream). The de-bundling does not depend on which suite you land in. The bound was not merely wrong about cost — it was pointed at the wrong OBJECT, which is the registry's own measurand axis firing on this file from outside. **WHAT THAT SCOPES IS THE COORDINATES ABOVE, NOT THE CAPABILITY — and the stronger reading was proposed here and REFUSED on primary source.** It is tempting to write that the packaged build "would not contain this code", and that reads as the capability being absent. Shupe et al. 2005 (ADASS XIV, ASP Conf. Ser., `2005ASPC..347..491S`) §4 says otherwise, verbatim: *"The Montage software (Laity et al. 2004) also uses Mink's routines and applies the coefficients"*, Mink's routines being WCSTools. **Both candidates are that one codebase**: Montage bundles `wcssubs3.9.0_montage`, and `libwcstools-dev`'s source package is `wcstools` with homepage `tdc-www.harvard.edu/software/wcstools/` — the same URL the SIP paper's own footnote points at (verified here from package metadata). So the SIP line references above are **upstream-bundled coordinates**, and the capability is a property of the library rather than of the bundling. **OPEN, and stated narrowly so nobody re-derives it wider:** whether wcstools 3.9.7 exposes the same SIP path as wcssubs 3.9.0, and whether Montage's `_montage` patches are inert with respect to it — a Montage-specific patch to the WCS library being exactly what would not survive de-bundling. UNVERIFIED: the paper is 2005 and describes Montage as of 2004, so it establishes lineage and design, not the state of either 2026 build. Not read: Debian's wcstools 3.9.7 source, and a diff of `wcssubs3.9.0_montage` against stock wcssubs 3.9.0. **AVAILABILITY, MEASURED ON THIS RIG (2026-08-16), because "absent from the audit" was handed over as if it were also absent from the distro and it is not:** `mProjectPP`/`mProject`/`mAdd`/`mImgtbl` are **not on PATH** and `montage` has **zero rows in `manifest.tsv`** — but the **BINARY** index (`apt-cache policy montage`) returns **Candidate 6.1+dfsg-2+b1** while the **SOURCE** index (`apt-cache showsrc montage`) reads **Version 6.0+dfsg-7** — two real indices that disagree, so name which one a version came from, `Section: science`, `Homepage: montage.ipac.caltech.edu`, *"Toolkit for assembling FITS images into mosaics"*, alongside **`python3-montagepy`** and `montage-gridtools`. **INSTRUMENT CAVEAT, and it is why this reading is sound in ONE direction only: `apt-cache policy` reads the BINARY INDEX, and this registry records it producing two false route-closures that way — SCAMP and PSFEx, both "NOT packaged", both wrong, with `apt-cache showsrc` as the completion. A POSITIVE survives that instrument** (a returned Candidate cannot be an index artefact, and `python3-montagepy` corroborates); a NEGATIVE from `policy` alone would not, and must not be written here. **A COLLISION WAS PREDICTED HERE AND IS REFUTED:** the obvious worry — that apt's `montage` is ImageMagick's unrelated `montage(1)`, the SWarp/`swarp` trap again — is FALSE; the packaged `montage` IS the astronomy toolkit. So this is not a procurement boundary and not a platform block; it is an uninstalled free tool sitting at the tier of this repo's largest measured defect (BACKLOG:`compose-homography-smear`). **NOT ADOPTED AND NOT RECOMMENDED HERE — what it needs first is a probe that its SIP handling is real on OUR headers, and a manifest row.** Whether to run that is not this file's call. |

**Why Montage is a NEVER-EVALUATED gap and not an oversight — and why nothing in
HEAD could have prompted a revisit.** Montage has existed in this tree three times
in its whole history (`git log --all -i -S'montage'`): added at **`df34ad1`**,
deleted at **`20b6c0d`** (*"retire(july14): wipe the exemplar"*), and this row. Its
entire prior life is ONE STRING — `git show aac7c18:datasets/july14/set-01/qa_work/registration_qa.json` (the retired july14 exemplar)
at `df34ad1`, `also_blocks`, verbatim: *"the WCS-reprojection route (SWarp / astropy
reproject / Montage) needs the same trustworthy per-frame distortion solution, so it
inherits this blocker."* So it was **named once as an unexamined member of a CLASS,
never probed and never rejected**, then removed as COLLATERAL when that dataset's
records were wiped — no decision about Montage was taken at any point, including the
one that deleted it. **The asymmetry is checkable rather than a judgement:** before
this row, `TOOLS.md` carried **SWarp on 7 lines, `reproject` on 9, and Montage on
ZERO** (`git show 27081db:TOOLS.md | grep -c`), from one sentence naming all three.
**AND THE SHARED BLOCKER HAS SINCE BEEN NARROWED, WITH NOBODY CARRYING IT BACK TO
THE CLASS:** *"no trustworthy per-frame distortion solution"* was measured on SINGLE
TRAILED FRAMES, and `docs/dead-ends.md` now records that Siril's own solver DOES
handle this class on STACKED members (`seqplatesolve -order=3`, 388 and 371 matched
stars). The class-level kill outlived its own narrowing, and because the only
mention had already died in an unrelated wipe, **nothing at HEAD could prompt the
revisit — `git show df34ad1` is the only recovery.** (Provenance: the HISTORIAN's,
CONFIRMED here by reading the blob rather than the commit subject. Bound it states
itself: `-S`/`-G` find a STRING in a diff, so a discussion that never wrote the word
is invisible to it.)
...
- **THE ASTROMETRIC GAIA CATALOGUE IS INSTALLED — this row previously said it was
  ...
  `8b4da51` with a hash chain and a functional probe (541 matched stars).
...
| **SCAMP** 2.10.0 (astromatic) | FREE | CLI | ✅ / ✅ / ✅ | **INSTALLED** — `/home/samsung/.local/bin/scamp`, `SCAMP version 2.10.0 (2020-12-01)`, built from Debian source. The prior "NOT PACKAGED on this distro" was the `apt-cache policy` error: it reported the BINARY repo and was quoted as availability. It is the removal condition for `object_tilt.py` and **the condition still does not fire** — verified on the built binary, not inferred: `scamp -d` offers astrometric `DISTORT_DEGREES`/`DISTORT_GROUPS`/`DISTORT_KEYS`/`STABILITY_TYPE`, while the photometric side has only `SOLVE_PHOTOM`, `MAGZERO_OUT/INTERR/REFERR`, `PHOTINSTRU_KEY`, `MAGZERO_KEY`, `PHOTOMFLAG_KEY` — **no photometric analogue of `DISTORT_DEGREES` exists**, so its photometric solution is a SCALAR per instrument and never the position-dependent field the condition names. Availability and capability are separate facts and only the first changed. **Before any SCAMP/SWarp arm, three defaults to set:** `ASTREF_CATALOG` defaults to `2MASS` — REMOTE — so set `FILE` against a local catalogue or the first run reaches a network service; `MOSAIC_TYPE UNCHANGED` (`SAME_CRVAL`, `SHARE_PROJAXIS` available) bears on members 4.28° apart; `ASTRINSTRU_KEY` (`FILTER,QRUNID`) defines the stability context. The reachable ORDER is an occupancy question, not a count: ~37 Tycho-2 matches per frame support order 1 by the Pan-STARRS yardstick (the occupancy row), but `STABILITY_TYPE INSTRUMENT` pools all exposures of an instrument into ONE distortion polynomial — ~480 matches over ~13 members against 300-for-order-4 — and the default `DISTORT_DEGREES` 3 needs 128; pooling helps only if the pooled coverage fills the (order+1)² grid, which members 4.28° apart with ~1000 px of drift make favourable and checkable before any arm. The pair is SCAMP (solves, writes `.head`) + SWarp (resamples); SWarp's own defaults to change are in its row. |
...
| **GraXpert deconvolution** (`deconv-obj` / `deconv-stellar` AI) | FREE | CLI + siril-native | ✅ / ✅ 🐢 (minutes CPU) / ✅ | **PRE-RELEASE only, and NOT AVAILABLE ON THIS RIG.** Official stable is **3.0.2 (BGE + denoise only)**; deconv exists in the 3.1.0-RC line and in the `geeksville` PyPI fork (`3.2.0a2`, not upstream). **NEITHER IS INSTALLED HERE — measured, positive-controlled:** `/opt` holds only `graxpert-3.0.2`, nothing named `graxpert` is on PATH, no pipx venv exists, the bundle's own `version.pyc` reads `3.0.2`, **the bundle carries no deconv module at all** (`find -iname '*deconv*'` returns 0 across it against 3 for `*denois*` as the control) and **the installed CLI offers `-cmd {background-extraction,denoising}` and nothing else.** Both tracked authorities agree: `manifest.tsv` pins `GraXpert 3.0.2` at the official Steffenhir zip, and `x86_bootstrap.sh` makes the fork an explicit opt-in it does NOT run — *"add pipx --pre 3.2.0a2 separately if deconv wanted"*. **So the ground is not that deconv does not exist; it is that it is not installed and not adopted here.** **PROVENANCE — the INSTALL was real on the retired arm64 box; this row went stale at the migration and is not a false-capability entry.** The arm64-retirement commit deleted this file's own *"ARM-VERIFIED on the base rig (probe, installed fork 3.2.0a2 + denoise model 3.0.2…)"* from the adjacent GraXpert **denoise** row, replaced it there with the hedge that still stands (*"that figure came from a fork build on the retired box"*), corrected the same claim in `CLAUDE.md` (*"GraXpert 3.2 at `~/.local/bin/graxpert`"*) — **and left THIS row untouched**: a claim corrected at its reporting site surviving one row away in the same file, for 13 days. NOT ESTABLISHED: whether deconv itself was ever exercised on that box — the surviving probe text names denoise, so the capability half may have been availability-only even where the package was installed. Real but undocumented CLI (`-cmd {deconv-obj,deconv-stellar}`); object-mode artifact bug **#243 open and unaddressed**. Nothing here is claimed about whether the fork's deconv works, and whether to install it is the owner's. BXT is the mature path. |
...
| **Anisotropic, spatially varying deconvolution** — the only class that would RECOVER corner detail | — | — | ❌ NOT PROCURED | Nothing installed does it: `torchmfbd` 0.9.2 (pip; A&A 2025 703 A269) states spatially variant PSFs — three DOCUMENTATION checks decide it, any one voids it: non-blind deconvolution with a SUPPLIED PSF field (unstated in its README); MOMFBD's premise is frame-to-frame aberration DIVERSITY while ours is STATIC and identical in every frame; PyTorch CPU-only at 6064×4040 with `torch` + `triton` costing 37 packages (`scripts/setup/requirements-tools.txt` — do not install to find out). `pyimcom` 1.2.1 (IMCOM, the principled generalisation of drizzle and homogenisation — a user-specified output PSF, not necessarily broader) mandates a survey `OBSFILE` observation-table schema with no plugin system and no bring-your-own-data path: weeks and a fork; `furry-parakeet` holds its linear-algebra kernels separately — the one cheap probe. Farrens et al. 2017 (A&A 601 A66) is built for a PSFEx-style varying PSF but its backend is a source integration; StarTools SVDecon is GPU + GUI; **BXT is NOT excluded by hosting — that reason was wrong and this file's own BXT row above refutes it: a standalone `rc-astro` v1.0.0 CLI plus a Siril script, no PixInsight host, Linux supported. And it IS spatially varying by the vendor's own account (BlurXTerminator Technical Manual): aberrations "vary across the field of view, with stars in the corners of an image rarely being as sharp as stars in the center", and BXT "uses stars in an image as PSF references in a local fashion ... apply[ing] different corrections to different parts of the image". So the class IS procurable at $99.95. THE CAVEAT IS PHYSICS, NOT LICENSING: BXT estimates its local PSF FROM STARS, and this corpus's corners are star-poor by the same scarcity that defeats a corner-true fit — so its correction is weakest exactly where the defect is worst, and that is what a trial has to measure;** `sf_deconvolve` deconvolves postage stamps one PSF per object — objects, not images; Cosmic Clarity is isotropic radius-only and attended-only (its row). Installed for PSF EVALUATION only: `galsim.des.DES_PSFEx` reads a PSFEx `.psf` and evaluates at any position (the basis-order trap cannot arise). Per-region tiling with an in-house blender is FORBIDDEN (pixel surgery on the deliverable). A single global PSF cannot remove the corner term: no field-constant trail scale on three grids (`datasets/aug06/corner_work/frame_depth.json`). BACKLOG:`corner-fix-landscape`. |
...
converts trail to anisotropy (`kappa_transfer.py`, since retired).
Answer: no.
...
**UNCONFIRMED, and this tier's own row says why: the flag is referenced in the
`full` branch ONLY, so under `separate` it may never be consulted. Probe the
frozen build before relying on it; byte-identical output across values reads as
"saturation confirmed" when it means the argument was never read.**
...
| **Siril `pm` / `rmgreen` / `satu` / `rgbcomp`** | FREE | siril-native | ✅ / ✅ / ✅ | **`rgbcomp -lum=` BLENDS IN HSL, AND SIRIL'S "L" IS NOT A LUMINANCE — the CLI documents neither.** From source: the RGB triple is converted to HSL, the computed lightness DISCARDED, the luminance file's value substituted wholesale, hue and saturation carried through. And `rgb_to_hsl` (`src/algos/colors.c`) computes `*l = (m + v) / 2.0` — the MIDPOINT of the min and max channels, i.e. geometric HSL lightness, NOT photometric luminance and NOT CIE L*, with no channel weighting and no ICC step. So this is the crude HSL-substitution form of LRGB rather than PixInsight LRGBCombination's CIE L*a*b* form, and substituting (min+max)/2 wholesale moves apparent SATURATION where a Lab join would not — most visibly on saturated pixels, where min and max are far apart. Verify on first use with an L corpus. | `pm` NBRGB/palette mixing (per-channel via separate mono images), `rmgreen` SCNR (kill SPCC's warned green cast), `satu` hue-targeted saturation, `rgbcomp` SHO/HOO assembly. Headless toolbox. |
...
**Cosmic Clarity DENOISE + darkstar only** (SHARPEN is ATTENDED — Qt modal, hangs
unattended, exit 134; see its Tier 5 row) + **ASTAP** (fast solve,
non-trailed class). A complete, competitive pipeline — and the chroma-noise gap's
FREE fill (Cosmic Clarity `--color_denoise_strength`) is UNCONFIRMED, see Tier 6. (`AstroSharp` is OUT — no Linux/CLI,
...
The inventory of record — every installed tool with its version, source,
checksum, path and verify command — is `scripts/setup/manifest.tsv`
(`scripts/qa/check_manifest_verify.sh` runs every row's verify); this section
carries the WHY behind each gap and each suite boundary, never a second copy of
the versions.

**THIS RIG'S BINARY AND SOURCE INDICES ARE DIFFERENT SUITES, AND EVERY "built from
Debian source" CLAIM HERE INHERITS THAT.** MEASURED 2026-08-16 from
`/etc/apt/sources.list*`: the binary line is **`kali-rolling`** (≈ Debian sid) and
**every `deb-src` line is `bookworm`** — there is no sid source line at all. So
`apt install <pkg>` and `apt-get source <pkg>` fetch from **different suites**, and
a reading that takes a version from one index and describes the other crosses that
boundary silently. Measured gaps, `apt-cache policy` against `apt-cache showsrc`:

| tool | binary, kali-rolling | source, bookworm |
|---|---|---|
| `source-extractor` | **2.28.2+ds-1** (installed) | **2.25.0+ds-3** |
| `montage` | 6.1+dfsg-2+b1 | 6.0+dfsg-7 |
| `swarp` | 2.41.5-3 (installed) | 2.41.5-1 |
...
that is correct and reproducible, since neither has a binary candidate at all.
SWarp's source reads in this file are bookworm `2.41.5-1` against an installed
`2.41.5-3`: same upstream version, different Debian revision, and the reads stand.
**Name the INDEX with any apt version, and when a source read is used to explain a
binary's behaviour, say whether they are the same suite.**
...
| **Siril 1.5.0-dev** | not installed; 1.4.4 is current stable | **ADOPT — and the benefit is larger than previously recorded.** DOCTRINE (1.5.0 docs): the mask BUILDER is **12 commands** — `mask_from_stars`, `mask_from_lum`, `mask_from_color`, `mask_from_channel`, `mask_threshold`, `mask_invert`, `mask_blur`, `mask_feather`, `mask_fmul`, `mask_autostretch`, `mask_bitpix`, `clear_mask` — and `-mask` is carried by ~30 commands, not six, including the stretches, the arithmetic set, and `rl [-loadpsf=] [-alpha=] [-iters=] [-stop=] [-gdstep=] [-tv] [-fh] [-mul] [mask]`, `sb`, `wiener`, `denoise`, `epf`. **CONSEQUENCE — a FORBIDDEN route is reclassified, not opened:** `docs/dead-ends.md` closes per-region PSF work as *"tiling and reassembly, i.e. pixel surgery on the deliverable"*, which rests on US doing the tiling; under 1.5 `rl -mask` with `mask_from_stars` makes it the tool's operation. **The bright-line objection dissolves; the PHYSICS one does not** — the registry's own *"the prior blocker is SNR, not seams"* stands untouched, so the route becomes **UNPROVEN, not permitted-and-recommended**. **LOAD-BEARING RISK, vendor-confirmed:** *"The old built-in Starnet interface has been removed… migrate to using `pyscript StarNet.py`"*; `render_tier.sh:263` emits `starnet -stretch` in a generated `.ssf`, so a bump breaks the shipped render tier. Mitigation: `pyscript` + `sirilpy` already run headless on 1.4.4, so the target mechanism is known. **AND `requires` GIVES NO PROTECTION — MEASURED on this rig: `requires 1.2.0` under siril 1.4.4 returns "OK! This script is compatible", so the argument is a FLOOR, not a pin; a 1.5 bump passes the version check and fails later on an unknown command.** LIMIT on that measurement: one observation establishes it accepts a NEWER siril than the argument and rules out exact-match; it does not establish there is no upper bound. Migrate before bumping (BACKLOG:`siril-1.5`). |
| **GraXpert classical interpolators** (RBF / Splines / Kriging via `-preferences_file`, no AI model) | **BLOCKED HEADLESS ON OFFICIAL BUILDS — the application is installed, the classical route is not reachable. MEASURED on this rig by EXECUTION, not read from source.** GraXpert's CLI CONSUMES classical settings and never GENERATES the points they need: `-cmd background-extraction -h` on the installed binary lists `-cli -cmd -output -preferences_file -gpu -v -ai_version -correction -smoothing -bg` plus `filename` and **no points flag**, and `-preferences_file`'s own help reads *"run all extraction methods based on a preferences file **that contains** background grid points"*. This rig's `~/.config/GraXpert/preferences.json` carries the whole classical schema (`interpol_type_option` RBF, `RBF_kernel`, `spline_order`, `bg_pts_option`, `bg_tol_option`, `smoothing_option`) and `background_points` **`[]`, length 0**. **`background_grid_selection` is NOT callable, and the reason is the BUNDLE rather than its absence:** `lib/graxpert/background_grid_selection.pyc` DOES ship in the official `graxpert-linux-amd64.zip` (sha256 `0a7364c3304ba19f12231d533c80b294054d6558d54ecd81668e4dec49092588`, the `manifest.tsv` row), and its parameter names are legible with `strings -n 2` — `data` / `num_pts_per_row` / `tol` / `sample_size`, contiguous — though `co_argcount` is NOT reachable that way, so the arity is CONSISTENT-WITH and not confirmed. It is **CPython 3.10 bytecode (magic 3439)** against host python3 **3.13.14 (magic 3571)**, so `import` returns `ImportError: bad magic number in 'graxpert'`. No 3.10 interpreter exists here (pyenv: system + 3.11.9), and the bundle ships `libpython3.10.so.1.0` — a LIBRARY, with no interpreter binary — so only GraXpert's own frozen launcher can load that bytecode, and it runs the app. **UPSTREAM (Steffenhir) SHIPS NO IMPORTABLE DISTRIBUTION — and the route is closed on ADOPTION rather than on existence, which is what makes the closure durable: an availability negative dies to one `pip index`, an adoption one does not.** `setup.py` at tag 3.0.2 is a **cx_Freeze** script whose `setup()` passes `name`, `version` and `executables` and **neither `packages` nor `py_modules`** (exact-anchor grep 0, positive-controlled against the keys it does declare) — it freezes an APPLICATION and declares no importable package. **BOUNDS, stated here rather than in a report that nobody re-reads: that mechanism is verified at tag 3.0.2 ONLY, and the asset sweep corroborating it covers 48 enumerated releases with the TAIL UNCHECKED** (no `.whl`, no `.tar.gz`, no `.egg` in any of them). So `pip install git+https://github.com/Steffenhir/GraXpert@3.0.2` never reaches an installed package: `graxpert/version.py` is literally `version = "SNAPSHOT"`, not PEP 440, and setup.py's module-scope `import astropy` / `from cx_Freeze import ...` block first in any environment lacking them — **which failure you hit is a property of your interpreter, not of the package**, so do not report one of them as the blocker. **The distributions that DO exist under the `graxpert` PyPI name are a FORK's**: `pip index versions graxpert` returns none, `--pre` returns `3.2.0a2 ... 3.2.0a0.dev6`, and `x86_bootstrap.sh` records that fork as *"NOT official, bug #243"* and does not install it. **THAT is the operative closure — the fork is not adopted here, and it is now EVIDENCED rather than asserted.** Re-derived from the wheel itself, digest-checked against PyPI's own published sha256 `4ca52d0a...e2f8b` BEFORE reading, nothing installed: it is a normal importable-SHAPED distribution — `Root-Is-Purelib: true`, `Generator: setuptools`, `top_level.txt` = `graxpert`, `graxpert/__init__.py` present, **44 `.py` / 0 `.pyc`**, which is the exact INVERSE of the official bundle's 0 `.py` / 23 `.pyc` frozen at magic 3439, **so the magic-number wall that closes the official route does not exist on the fork at all** — and it DOES ship `graxpert/background_grid_selection.py`, 1859 bytes. **What is NOT measured is that the import SUCCEEDS at runtime: that is a separate mode and nothing here tests it.** The closure is unaffected either way, because it is adoption and not availability — which is precisely why it cannot be falsified by an index query. Its PyPI author, email and homepage are inherited upstream metadata, so only `project_urls.Source Code` distinguishes it. The `--pre` filter is what decides the first answer, controlled on ONE package: `graxpert` empty, `graxpert --pre` found, `numpy` found, `numpy --pre` found. | **BLOCKED HEADLESS, and the sentence this replaces INVERTED its own conclusion.** It read *"Points can be produced by GraXpert's own `background_grid_selection(data, num_pts_per_row, tol, sample_size)`, so the route needs no in-house analysis."* On official builds the classical operators require the points to be SUPPLIED, and supplying them **IS** the in-house analysis on the deliverable's pixels that sentence existed to rule out — `CLAUDE.md`'s FORBIDDEN case, reached by following the toolkit. **PROVENANCE: the claim entered on a SOURCE READ and no import was ever executed** — `git log --all -S 'import graxpert'` returns nothing in the entire history, and the seed commit's own message says *"MEASURED from source at tag 3.0.2"*. Its author DID name the gap in the same commit (*"NOT CHECKED: that `background_grid_selection` imports from a pip install rather than only the bundled app"*), so this is not an inference made silently: **the defect is that the hedge sat in THIS cell while the capability assertion sat in the availability cell 1,211 characters away, across a column boundary** — a reader who correctly column-splits a table-borne claim reads the assertion without the hedge. That is why the verdict now lives in the availability cell. MECHANISM, untested, and it SURVIVES: the AI path infers a background CLASS and absorbs frame-filling structure with no knob to refuse it; RBF/Splines/Kriging have no notion of background and fit a surface through the points supplied, so absorption depends on placement and stiffness — a failure that is inspectable and controllable where the AI one is not. **LIMIT: auto-placement would not escape it either** — `background_grid_selection` imports `find_darkest_quadrant`, so on a frame-filling starlight field the darkest cells are still starlight. **The one path that keeps the route alive without in-house code: generate the points in the GUI, save the preferences file, and feed that file to the CLI — ATTENDED, not headless**, the same disposition this file already applies to Cosmic Clarity's sharpen. UNDETERMINED and unchecked: whether the GUI exposes point generation, and whether such a file is portable across images. **The CLI default is AI**, so the recorded ~2/3 absorption was the default path and not the classical one. PROBE (specified, unrun, and it needs the points question settled first): one knob on the same stack — default AI vs `-preferences_file` RBF at stated density/tolerance — judged on NAN-region contrast %R/G/B against the recorded 8.5/2.9/5.6 (own sky flat) and 2.4/0.7/2.1 (AI Division). **Nothing here is a procurement recommendation.** |
...
**SWarp `-HEADER_ONLY Y`, three runs differing only in the input header's distortion — the evidence for the SWarp row above:
