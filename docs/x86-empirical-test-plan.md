...
| 1 — orchestration port | tool-sourced measures re-run on this rig; every divergence/gap-filler declares its removal condition in its own docstring; `frame_metrics` CFA condition FIRED and honoured (debayer at convert; +9.1% FWHM inflation measured on the CFA arm); sirilpy headless via `.ssf`→`pyscript` probe-confirmed | the declaring files' own conditions; `CLAUDE.md` Environment |
