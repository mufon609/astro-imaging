...
| bit depth | 32-bit; docs warn 16-bit stacking loses information | 16-bit stack-time intermediates (`set16bits`) — arm-era adaptation, measured ≈18× below per-frame noise (~+0.3% stack noise) | **DIVERGENCE whose removal condition has now FIRED** (x86 RAM/disk present) → drop `set16bits`, re-measure, declared delta (BACKLOG) |
...
- Repo records: `datasets/july23/*` (QA/audit/solves), `docs/dead-ends.md`
  (SIP/solver/weighting mechanisms), `docs/siril-stacking-workflow.md` (1.4.4
  syntax audit this doc builds on)
