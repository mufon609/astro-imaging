...
| file | status |
|---|---|
| `siril-behaviors.md` | DONE, audited + decluttered at `9957337`: 13 → 13 entries, 276 → ~210 file lines; SPCC-segfault entry compressed to elimination kernel + `CLAUDE.md` Environment as the mechanism home; `update_key` fix stated as shipped (`header_apply_keys` verified in `stamp_headers.sh:408/:427`); the 67-line offset/stat block kept all its measured probes with incident narrative and sirilpy annotation forensics compressed (the bright-line pixeldata closure kept verbatim); zero-weight-fallback kept as properly-hedged DOCTRINE with its settling probe; idiv scalar-record corollary kept; top-down-frame mirror-correction kept with both 400/400 + 300/300 measurements; no inbound file pointers existed to constrain the pass |
| `plate-solving-wcs.md` | DONE, audited + decluttered at `9957337`: 8 → 8 entries, 242 → ~195 file lines; implemented-fix compressions verified in the shipped code (hint-contradiction gate exit 9 + FLOOR-CLASS warning; `--central` semantics homed in `solve_field.py`'s docstring; `finish_render.sh --max-stars=` with the preserved-not-endorsed comment; the dual-matrix strip in `inject()` with the mechanism in its code comment, 34 products backfilled); the mildly-trailed boundary probe pointer added (BACKLOG:`native-solve-and-sip`); a pre-existing ranking numbering gap fixed ((1)(2)(4) → (1)(2)(3)); the WCS-misleads enumeration consolidated to one five-way list; solver-on-stacked-members kept verbatim as the supersession anchor two files point at |
...
| final batch: `measurement-discipline` 203→~150, `separation-deconv-psf` 179→~120, `stretch-colour-judgment` 174→~150, `background-extraction` 135→~110, `00-registry-contract` 143→~135, `acquisition-checklist` 120→~110, `terminology-dust` 82→~75, `lunar-planetary` 73→~70, `intake-frame-qa` 40→~36 | DONE, audited + decluttered at `9957337`, one commit. Notable: two dead live-ledger citations repaired to the `git show c7db472:` + INHERITED form (`warp_load_determinism.json`, `rebuild_repeat_floor_set01`, plus `background_residual_decomposition` in the already-passed calibration file — all three swept in the july31 raws-only reset, survival at c7db472 verified); the stale `BACKLOG:optical-state-models` pointer in the acquisition checklist replaced with the live `lens_models.json` authority; the field-variable-PSF title-rot saga compressed to its three rot-proof grounds; the contract's owner-ratified rules kept verbatim-adjacent with only narrative/attribution trimmed, and its dangling "pattern entry under QA/scope" pointer resolved to the in-file instances |

**PHASE 2 COMPLETE — all 17 files audited and decluttered.** `split.py`'s
byte check is now vacuous (nothing is phase-1); the manifest remains the
provenance map from original block to file. What remains is the MERGE
decision (owner): replace `docs/dead-ends.md` with an index into this
directory per the constraints below.
