...
| `nikon-test/` | Nikon D810A · 180 mm f/2.8 + 50 mm f/4 · ISO 800 · 181 s | `lmc_180mm` · `smc_180mm` (**matched-flat**) · `wide_50mm` (**self-flat**) · `nebula_180mm` (mosaic panel — single-field pipeline skips it) | **`lmc_180mm` + `smc_180mm` DONE** (matched-flat; drove the corridor removal). Both: 12–13/13 registered, solved, SPCC (southern Gaia chunks fetched via `spcc_cone.py`), new composition-agnostic gate PASS. Renders honest — the set-03-tuned `chroma_core`/`satu` desaturate real colour + over-saturate the SMC's OIII (per-dataset recipe = BACKLOG C1). `wide_50mm` (self-flat) + `nebula_180mm` queued. | `nikon-test/README.md` (gitignored — Wei-Hao Wang's archive, practice-only, never commit/redistribute) |
